# v2 of latent_nn_clean contains histogram plots weights
...
param_init_scale=1
sample_size_func=80 # number of samples to use to approximate average pi
num_func=200 # number of functions to sample
...
if __name__ == '__main__':
    ...
    num_context = 1
    iters=500
    weights_mat=np.zeros((iters, truncation_level)) # this matrix stores the weights at each iteration

    encoder_arch = [dimx+dimy, 8,8, 2*dimz]
    decoder_arch = [dimx + truncation_level, 20, 20, dimy]
    ...
    def callback(params, t, g):
        plot_inputs = np.linspace(-8, 40, num=800)[:, None]
        preds = sample_functions(params, plot_inputs, data, num_func) # use full dataset

        # sample weights at each iter
        decoder_params, encoder_params = params
        data_moments = nn_predict_encoder(encoder_params, pack(data))
        log_a_encoder = np.mean(data_moments[:, :(truncation_level - 1)], axis=0)
        log_b_encoder = np.mean(data_moments[:, (truncation_level - 1):], axis=0)

        #temp_sample=np.mean(sample_latent_pi(log_a_encoder, log_b_encoder, plot_inputs.shape[0]), axis=0) * 100
        temp_sample = np.mean(sample_latent_pi(log_a_encoder, log_b_encoder, sample_size_func), axis=0) * 100
        print("ONE SAMPLE MEAN OF STICKS IN PERCENT : ", temp_sample)
        weights_mat[t,:]=temp_sample
        ...
        for k in range(truncation_level):
            ax1.plot(weights_mat[0:t,k])

        #ax.plot(x.ravel(), y.ravel(), 'o')
        ax.plot(xf.ravel(), yf.ravel(), '.', color='black')
