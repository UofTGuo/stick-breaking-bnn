...
### HYPERPARAMS ###
ystar_log_sigma=20
elbo_samples=20 # number of times to sample q(z|context,test) in ELBO calc for MC calculation
###################
...
def sample_functions(params, inputs, cond_data, num_functions):
    ...
    fs = [decoder_predict(decoder_params, inputs, sample_latent(encoder_params, cond_data, inputs.shape[0]))[:,:1] for _ in
          range(num_functions)]
...
if __name__ == '__main__':
    ...
    decoder_arch = [dimx+dimz, 20, 20, 2*dimy]
    ...
    init_encoder_params = init_net_params(encoder_arch, scale=0.5)
    init_decoder_params = init_net_params(decoder_arch, scale=0.5)
