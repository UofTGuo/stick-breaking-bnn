...
def sample_functions(params, inputs, data, num_functions):
    ...
    log_a_encoder = data_moments[:, :1]
    log_b_encoder = data_moments[:, 1:]
...
def lower_bound(decoder_params, encoder_params, data):
    ...
    for c in range(elbo_samples):
        ...
        latent=sample_latent_pi(log_a_encoder, log_b_encoder, log_a_encoder.shape[0])
...
if __name__ == '__main__':
    ...
    encoder_arch = [dimx+dimy, 32,32, 2*dimz]
    #decoder_arch = [dimx+dimz, 8, 8, 2*dimy]
    decoder_arch = [dimx + truncation_level, 32,32, 2 * dimy]
    ...
    def callback(params, t, g):
        ...
        ax.plot(xf, preds, color='grey', linewidth=0.1)
